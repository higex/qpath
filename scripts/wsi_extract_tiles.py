"""
WSI_EXTRACT_TILES: extract a number of tiles (rectangular regions) from an image.
"""
...
def main():
    p = opt.ArgumentParser(description="""
        ...
        the tiles are extracted only within the regions covered by the mask.
        """)
    p.add_argument('img_file', action='store', help='RGB image file of an H&E slide')
    p.add_argument('out_file', action='store', default='tiles.dat',
    ...
    X = np.stack(tiles, axis=3)
    np.savez_compressed(args.out_file, tiles=X)
